...
def rel_to_abs(x):
    ...
    flat_x = torch.reshape(x, (B, Nh, L * 2 * L)).cuda()
...
def botnet(pretrained=False, **kwargs):
    """
    Bottleneck Transformers for Visual Recognition.
    ...
    if pretrained:
        state_dict = torch.load(r'D:\app\vscode\py\paddle\test\botnet50.pth.tar', map_location=torch.device('cpu'))
